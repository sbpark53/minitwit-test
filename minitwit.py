...
@app.route('/<username>/logincheck')
def loginCheck(username):
    """login check2"""
    if g.user:
        flash('"%s" login check' % username)
    else:
        flash('login please')
    return redirect(url_for('check', username=username))

@app.route('/logincheckk')
def loginCheckk():
    """login check2"""
    if not g.user:
        flash('login please')
...
def timeline():
    ...
    redirect to the public timeline.  This timeline show the user's
